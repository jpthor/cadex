...
for (const component of wingevons) {
  ...
  assert.ok(absOuterSpanM > 1.85 && absOuterSpanM < 1.95, `${component.name} must end at the drawn tip station`);
  assert.ok(component.bounds.min[0] < -1.55, `${component.name} must include the drawn trailing edge`);
  assert.ok(component.bounds.max[0] < -0.62 && component.bounds.max[0] > -0.72, `${component.name} must include the drawn leading edge`);
