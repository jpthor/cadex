...
approx(result.staticThrustToWeight, 1.6, "fallback static thrust-to-weight is computed, not user-entered");
approx(result.adjustedTakeoffThrustToWeight, 1.6, "fallback takeoff thrust-to-weight is hover-efficiency adjusted");
approx(result.availableThrustToWeight, result.adjustedTakeoffThrustToWeight, "available thrust-to-weight uses hover-adjusted takeoff value");
...
approx(result.hoverPropEfficiency, effectivePropEfficiency, "fallback hover efficiency uses the actuator-disk efficiency estimate");
approx(result.cruisePropEfficiency, effectivePropEfficiency, "fallback cruise efficiency uses the same estimated prop efficiency");
...
const sampleRequiredHoverThrustPerMotorN = aircraftMassKg * 9.80665 / motorCount;
const sampleRequiredHoverInducedVelocityMS = Math.sqrt(sampleRequiredHoverThrustPerMotorN / (2 * 1.225 * sampleDiskAreaM2));
const sampleIdealHoverPowerPerMotorW = sampleRequiredHoverThrustPerMotorN * sampleRequiredHoverInducedVelocityMS;
const sampleHoverPowerPerMotorW = samplePowerPerMotorW * Math.pow(sampleRequiredHoverThrustPerMotorN / sampleThrustPerMotorN, 1.5);
const sampleHoverEfficiency = Math.min(0.86, Math.max(0.2, sampleIdealHoverPowerPerMotorW / sampleHoverPowerPerMotorW));
const sampleCruiseEstimateMS = samplePitchSpeedMS * samplePropeller.peakEfficiency * 0.75;
const samplePeakEfficiencySpeedMS = samplePropeller.peakEfficiencyMph * 0.44704;
const sampleCruiseSpeedMismatch = Math.abs(sampleCruiseEstimateMS - samplePeakEfficiencySpeedMS) / samplePeakEfficiencySpeedMS;
const sampleCruiseEfficiency = Math.min(0.9, Math.max(0.25, samplePropeller.peakEfficiency * (1 - Math.min(0.32, sampleCruiseSpeedMismatch * 0.28))));
...
approx(sampleResult.hoverPropEfficiency, sampleHoverEfficiency, "selected propeller hover efficiency uses static thrust and disk loading");
approx(sampleResult.cruisePropEfficiency, sampleCruiseEfficiency, "selected propeller cruise efficiency uses forward-flight peak efficiency");
...
approx(sampleResult.staticThrustToWeight, sampleTotalThrustN / (aircraftMassKg * 9.80665), "selected propeller static thrust-to-weight is computed from available thrust");
approx(sampleResult.adjustedTakeoffThrustToWeight, sampleResult.availableThrustToWeight, "selected propeller available T/W is the adjusted takeoff value");
assert.ok(sampleResult.adjustedTakeoffThrustToWeight <= sampleResult.staticThrustToWeight + 1e-9, "hover efficiency cannot increase takeoff T/W above static data");
