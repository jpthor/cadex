...
import { batteryMassFromSizing, batterySamples, computePropulsionSizing, findBestPropulsionCombo, motorSamples, propellerMassEstimate, propellerSamples, rotorDefinitionFromSizing } from "./propulsionEngine";
...
type AppMode = "design" | "propulsion" | "size";
      ...
      <footer className="timeline">
        ...
        <>
          ...
          <footer className="timeline size-footer">
            ...
            <div className="timeline-events">
              ...
              <span>{sizingProject.mission.motorCount} motors</span>
            </div>
          </footer>
        </>
      ) : (
        <>
          ...
          <footer className="timeline size-footer">
            ...
            <div className="timeline-events">
              <SizingSummaryFooter analysis={liveSizingAnalysis} />
            </div>
          </footer>
        </>
      )}
...
function PropulsionWorkspace({
  ...
  const [selectedMotorId, setSelectedMotorId] = useState(() => motorSamples[5]?.id ?? motorSamples[0].id);
  const [selectedPropellerId, setSelectedPropellerId] = useState(() => propellerSamples[8]?.id ?? propellerSamples[0].id);
  const [selectedBatteryId, setSelectedBatteryId] = useState(() => batterySamples[5]?.id ?? batterySamples[0].id);
  const [targetThrustToWeight, setTargetThrustToWeight] = useState(1.3);
  const [targetEnduranceMin, setTargetEnduranceMin] = useState(20);
  ...
  function findBestCombo() {
    ...
    if (!best) return;
    setSelectedMotorId(best.motor.id);
    setSelectedPropellerId(best.propeller.id);
    setSelectedBatteryId(best.batteryPack.id);
      ...
      <section className="propulsion-panel propulsion-summary-panel">
        ...
        <div className="propulsion-optimizer">
          <PropulsionNumberField
            ...
            onChange={setTargetThrustToWeight}
          />
          <PropulsionNumberField
            label="Min endurance"
            suffix="min"
            ...
            onChange={setTargetEnduranceMin}
          />
      ...
      <section className="propulsion-panel">
        ...
        <div className="propulsion-input-grid">
          <PropellerSelect
            ...
            onChange={setSelectedPropellerId}
          />
        </div>
        <div className="propulsion-readouts">
...
function applyPropulsionMassesToSizing(
  sizing: SizingProject,
...
function formatInchesAsMm(valueIn: number) {
  return `${(valueIn * 25.4).toFixed(0)} mm`;
}

function formatMetersAsMm(valueM: number) {
  return valueM > 0 ? `${(valueM * 1000).toFixed(0)} mm` : "not set";
}

function PropulsionNumberField({
  label,
  suffix,
  step,
  value,
...
function ResultGroup({ children, title }: { children: ReactNode; title: string }) {
  return (
    <div className="propulsion-result-group">
      <h3>{title}</h3>
      <div>{children}</div>
    </div>
  );
